; Examples of programming with fragment shaders
; I've made the shaders in Shadertoy and then I've ported them here, that's why they have the structure they have.
...
Procedure BuildCurrentShader()
 Protected objects.sgl::ShaderObjects
 Protected vs, fs
 Protected fragFile$ = gShadersList()
 
 If gShader
    sgl::DestroyShaderProgram(gShader)
 EndIf
 
 vs = sgl::CompileShaderFromFile("Common.vs", #GL_VERTEX_SHADER) 
 sgl::AddShaderObject(@objects, vs) 
 ASSERT(vs)
 
 fs = sgl::CompileShaderFromFile(fragFile$, #GL_FRAGMENT_SHADER) 
...
Procedure SelectNextShader()
 If NextElement(gShadersList()) = 0
    FirstElement(gShadersList())
 EndIf
EndProcedure

Procedure BuildShadersList()
 Protected ed

 ed = ExamineDirectory(#PB_Any, "./", "*.fs")
  
 While NextDirectoryEntry(ed) 
    If DirectoryEntryType(ed) = #PB_DirectoryEntry_File
        AddElement(gShadersList())
        gShadersList() = DirectoryEntryName(ed)
    EndIf
 Wend
 
 FinishDirectory(ed)
 
 SortList(gShadersList(), #PB_Sort_Ascending)
...
Procedure Render()
 ...
 Protected.vec3::vec3 iResolution
 Protected u_projection, u_iResolution
 ...
 u_projection = sgl::GetUniformLocation(gShader, "projection")
 sgl::SetUniformMatrix4x4(u_projection, @projection)

 vec3::Set(iResolution, w, h, 1.0) ; w x h plus pixels squareness (1.0 for square pixels)
 
 u_iResolution = sgl::GetUniformLocation(gShader, "iResolution")
 sgl::SetUniformVec3(u_iResolution, @iResolution)
...
Procedure MainLoop() 
 While sgl::WindowShouldClose(gWin) = 0
    ...
    If sgl::GetKeyPress(sgl::#Key_ENTER)
        SelectNextShader()
        BuildCurrentShader()
    EndIf
